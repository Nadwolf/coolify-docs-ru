...
description: Как зафиксировать конкретные минорные и патч-версии Node.js в Nixpacks, используя переопределение архива nixpkgs.
---

# Версионирование Node.js в Nixpacks

Nixpacks поддерживает указание только **мажорной версии** для Node.js. На этой странице объясняется данное ограничение и предлагается способ фиксации конкретных минорных и патч-версий.

## Понимание ограничения

Согласно [документации провайдера Node в Nixpacks](https://nixpacks.com/docs/providers/node?utm_source=coolify.io):

> «Можно указать только мажорную версию. Например, `18.x` или `20`.»

Когда вы указываете версию Node.js через:

- переменную окружения `NIXPACKS_NODE_VERSION`
- поле `engines.node` в `package.json`
- файлы `.nvmrc` или `.node-version`

Вы можете управлять только **мажорной версией** (например, `20`, `22`). Конкретная минорная и патч-версия (например, `22.13.1` против `22.14.0`) определяется архивом nixpkgs, который Nixpacks использует внутри себя.

## Проблема

Вы устанавливаете `"node": ">=22"` в `package.json` или `NIXPACKS_NODE_VERSION=22`, но вашему приложению требуется именно Node 22.14.0. Nixpacks вместо этого может предоставить 22.12.0 или другую патч-версию из своего стандартного архива nixpkgs, что может привести к ошибкам во время выполнения, если ваш код зависит от функций из более свежего патча.

## Решение

Чтобы зафиксировать конкретную минорную/патч-версию, вы можете переопределить архив nixpkgs в файле `nixpacks.toml`. Создайте этот файл в корне вашего репозитория:
...
Значение `nixpkgsArchive` — это SHA-хеш коммита из [репозитория NixOS/nixpkgs](https://github.com/NixOS/nixpkgs?utm_source=coolify.io). Каждый коммит содержит определённые версии пакетов, поэтому, фиксируя коммит, вы управляете точными версиями.

## Поиск нужного коммита архива

Чтобы найти коммит nixpkgs, содержащий нужную вам версию Node.js:

1. Перейдите в [репозиторий NixOS/nixpkgs](https://github.com/NixOS/nixpkgs?utm_source=coolify.io).
2. Найдите файлы вроде `v20.nix`, `v22.nix` или `v24.nix` (файлы определений версий Node.js).
3. Проверьте историю коммитов, чтобы увидеть, когда была добавлена нужная вам версия.
4. Скопируйте полный SHA коммита.

**Некоторые проверенные коммиты для Node.js:**

- `51ad838b03a05b1de6f9f2a0fffecee64a9788ee` → Node 22.13.1
- `bf744fe90419885eefced41b3e5ae442d732712d` → версии Node 22.x
- `ffeebf0acf3ae8b29f8c7049cd911b9636efd7e7` → Node 22.14.0 (ветка unstable)

Подробнее в документации Nixpacks: [nixpkgs archive](https://nixpacks.com/docs/configuration/file#nixpkgs-archive?utm_source=coolify.io).

## Справочник версий Node.js (SHA256 хеши)

В следующих таблицах перечислены версии Node.js и их SHA256 хеши из истории nixpkgs. Это поможет вам убедиться, что вы получаете ожидаемую версию при работе с архивами nixpkgs.

::: warning ИСПОЛЬЗУЙТЕ НА СВОЙ СТРАХ И РИСК
Эти хеши предоставлены только для справки. Они не были проверены на точность или стабильность. Тщательно протестируйте перед использованием в продакшене.
:::
